...
from data_utils.camera_io import compute_vertex_depth, load_matrix_data, project_3d_to_2d_cpp_exact
...
def load_geometry_like_dataset(filepath: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Mirror GeometryDataset._load_geometry while preserving raw values for projection."""
...
def load_vertex_texture_colors(
    ...
    uv = load_combined_mesh_uv(model_dir="assets/topology", copy=True).astype(np.float32, copy=False)
    if uv.shape[0] != vertex_count:
        mesh_indices_path = Path("assets/topology/mesh_indices.npy")
